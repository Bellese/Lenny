...
# The seeded H2 database is baked in by the bake-hapi-image.yml
...
# shadow the baked H2 store committed by the bake workflow.
#
# The Lucene index under /data/hapi/lucene is NOT usefully baked: it commits as an
# empty index (a 69-byte segments_1 plus a 0-byte write.lock per index), because the
# bake's bare `docker stop` gives the JVM only Docker's default 10s SIGTERM window —
# H2's MVStore is crash-durable and survives, Hibernate Search never commits and does
# not. This costs less than it appears: HAPI serves reference/token/date search params
# from the relational HFJ_RES_LINK / HFJ_SPIDX_* tables inside H2, so patient-scoped
# search works at first boot. Lucene backs full-text and terminology only (#425).
