...
### Added
- **End-to-end Jobs pipeline validation** — New parametrized integration test (`test_full_jobs_pipeline.py`) runs all 11 connectathon measures through the Lenny Jobs API against the prebaked HAPI stack and asserts per-patient numerator/denominator outputs match the ground-truth expected populations from the connectathon bundles. Closes the gap between `test_connectathon_measures.py` (bypasses Lenny) and `test_full_workflow.py` (no count assertions). CMS1017 (HTTP 400 from HAPI) and CMS529 (manifest ID mismatch tracked in issue #253) are skipped with documented reasons; 8 measures pass, 3 have known prebaked-image failures tracked in issue #256.
